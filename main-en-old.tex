...
If $F\subseteq E$, we write $G\setminus F\coloneqq (V,E\setminus F)$ and $V[F]\coloneqq \{v\in V:\ \exists\, e\in F \text{ with } v\in e\}$. For a vertex $v\in V$, its open and closed neighborhoods are $N(v)\coloneqq \{w\in V:\ \{v,w\}\in E\}$ and $N[v]\coloneqq N(v)\cup\{v\}$. If $P=(v_0,\dots,v_k)$ is a path and $0\le i\le j\le k$, then $P[v_i,v_j]\coloneqq (v_i,\dots,v_j)$.

\begin{definition}[Contraction of a non-empty vertex set]\label{def:contraction}
Let $\emptyset\neq W\subseteq V$ and let $w^\ast\notin V$ be a new vertex. Following the standard contraction operation for simple graphs~\cite[Section~1.7]{diestel}, define $\pi_W\colon V\to (V\setminus W)\cup\{w^\ast\}$ by $\pi_W(v)=w^\ast$ for $v\in W$ and $\pi_W(v)=v$ for $v\notin W$. The graph $G/W$ has vertex set $(V\setminus W)\cup\{w^\ast\}$ and edge set $E(G/W)=\bigl\{\{\pi_W(x),\pi_W(y)\}: \{x,y\}\in E,\ \pi_W(x)\neq \pi_W(y)\bigr\}$. If $e=\{x,y\}\in E$ and $\pi_W(x)\neq \pi_W(y)$, we write $e/W\coloneqq \{\pi_W(x),\pi_W(y)\}\in E(G/W)$. For $F\subseteq E$, we also put $F/W\coloneqq \bigl\{\{\pi_W(x),\pi_W(y)\}: \{x,y\}\in F,\ \pi_W(x)\neq \pi_W(y)\bigr\}\subseteq E(G/W)$. By construction, loops are discarded and parallel edges are identified, so $G/W$ is again simple.
\end{definition}
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
  ...
  \begin{scope}[xshift=7.4cm]
    ...
    \node[circle,draw,inner sep=2pt,fill=gray!50,label=above:$w^\ast$] (wp) at (1.8,1.5)  {};
    \draw (xp)--(wp) (wp)--(yp) (wp)--(zp) (yp)--(zp);
  \end{scope}
\end{tikzpicture}
\caption{Contracting a non-empty vertex set $W$ to a single vertex $w^\ast$. Internal edges of $W$ disappear, while edges leaving $W$ become edges incident with $w^\ast$.}
\label{fig:contraction-definition}
\end{figure}
...
We shall use without further proof the following standard facts from graph theory; see Diestel~\cite[Sections~1.3 and~1.7]{diestel}. Every walk contains a path with the same endvertices and no greater length, every subpath of a shortest path is shortest between its own endvertices, and the projection of a walk under a contraction yields, after deleting consecutive repetitions, a walk in the contracted graph whose length does not exceed the original one.
...
We claim that $F^-$ is semi-cutting for $T^-$ in $G$. If $|T^-|\le 1$, then the claim is vacuous. Assume from now on that $|T^-|\ge 2$. By Observation~\ref{obs:finite-terminal-distances}, every pair of distinct vertices in $T^-$ has finite distance in $G$. It is therefore enough, by Observation~\ref{obs:shortest-paths}, to show that every shortest path between two distinct vertices of $T^-$ meets $F^-$.

Take distinct $u,w\in T^-$, and let $P$ be a shortest $u$--$w$ path in $G$. Since $u,w\in T$, Observation~\ref{obs:shortest-paths} gives $E(P)\cap F\neq\emptyset$. If $e\notin E(P)$, then already $E(P)\cap F^-\neq\emptyset$. Suppose now that $e\in E(P)$. Write $P=(v_0,\dots,v_m)$ and let $v_j=t$. Because $u,w\neq t$, we have $0<j<m$. Put $P_u\coloneqq (v_0,\dots,v_j)$ and $P_w\coloneqq (v_j,\dots,v_m)$, so Figure~\ref{fig:split-path} depicts the situation. By the standard subpath property, $P_u$ and $P_w$ are shortest paths for the pairs $(u,t)$ and $(t,w)$. Since $e\in E(P)$ and $e$ is incident with $v_j=t$, either $e=\{v_{j-1},v_j\}$ or $e=\{v_j,v_{j+1}\}$.
...
For two terminals, semi-cutting sets are exactly edge transversals of the family of geodesics between a prescribed pair of vertices. This places the notion next to the vital-link and vital-edge literature initiated by Corley and Sha~\cite{corley-sha} and continued by Malik, Mittal, and Gupta~\cite{malik-mittal-gupta} and by Nardelli, Proietti, and Widmayer~\cite{nardelli-proietti-widmayer}. The common feature is that deleting a distinguished set of edges must force a shortest-path obstruction. The difference is structural: those papers focus on one designated terminal pair, whereas a semi-cutting set must obstruct every terminal geodesic simultaneously.

For larger terminal sets, the closest classical comparison is the multiterminal cut problem of Dahlhaus, Johnson, Papadimitriou, Seymour, and Yannakakis~\cite{dahlhaus1994}. Their setting requires complete separation of the terminals, while semi-cutting asks only for a strict increase of every terminal-terminal distance. Hence every multiterminal cut is semi-cutting whenever the original terminal distances are finite, but the converse fails because a semi-cutting set may leave every pair of terminals connected. Theorem~\ref{thm:main} may therefore be read as a universal lower bound that survives even after complete separation is relaxed to strict distance increase.
...
These comparisons leave two natural structural questions. First, one may ask for a characterization of the extremal triples $(G,T,F)$ with $|F|=|T|-1$. Second, one may ask how rigid minimum semi-cutting sets must be inside the geodesic structure determined by the terminal set.
